...
## Load data
readmit_data <- read_csv("inst/sample_readmit_data.csv")
...
format_prediction_to_percent <- function(sheet, pos){
  ...
  range_spec <- googlesheets4:::as_range_spec(
    #LETTERS[c(x$sheets$grid_columns[2]-1, x$sheets$grid_columns[2])],
    pos,
    sheet = 2,
...
## Load googlesheet where we store patient data
sheet_id <- "https://docs.google.com/spreadsheets/d/13foVot70J2A4Ku3JonTRbTp4E3UgB6_FTAOHFDIKHb4/edit?usp=sharing"

## Read in data

## Input sample data into the google sheet
#patient_data <- readmit_data
...
if(nrow(new_patients) > 0){
  ...
  send_to <- c("aoidatscimentoring@gmail.com")

  emayili <- envelope() %>%
    from("arimoroolayinka@gmail.com") %>%
    to(send_to) %>%
